...
static int smb1351_chg_otg_regulator_enable_asus(struct smb1351_charger *chip)
{
	int rc = 0;
...
static int smb1351_chg_otg_regulator_disable_asus(struct smb1351_charger *chip)
{
	int rc = 0;
...
static int smb1351_charger_reg_dump_proc_read(struct seq_file *buf, void *data)
{
	dump_buf = buf;
	dump_regs(smb1351_charger_dev);
	dump_buf=NULL;
	return 0;
}

static int smb1351_charger_reg_dump_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, smb1351_charger_reg_dump_proc_read, NULL);
}

static const struct file_operations smb1351_charger_reg_dump_fops = {
	.owner = THIS_MODULE,
	.open = smb1351_charger_reg_dump_proc_open,
	.read = seq_read,
};
static void create_smb1351_charger_reg_dump_proc_file(void)
{
	struct proc_dir_entry *smb1351_charger_reg_dump_proc_file = proc_create(smb1351_charger_reg_dump_PROC_FILE, 0666, NULL, &smb1351_charger_reg_dump_fops);

	if (smb1351_charger_reg_dump_proc_file) {
		printk("[BAT][CHG][SMB][Proc]smb1351_charger_reg_dump_proc_file create ok!\n");
	} else{
		printk("[BAT][CHG][SMB][Proc]smb1351_charger_reg_dump_proc_file create failed!\n");
	}
}
...
struct smb1351_charger msb1351_dev;
static struct proc_dir_entry *chargerIC_status_proc_file;
static int chargerIC_status_proc_read(struct seq_file *buf, void *v)
{
	int ret = -1;
	u8 reg;
	ret = smb1351_read_reg(&msb1351_dev, 0x36, &reg);
	printk("charger ic 0x%02x reg:%x\n",0x36,reg);
	if (ret == 0) {
		//i2c charger ic exist 
		seq_printf(buf, "1\n");
	} else{
		//i2c charger ic no exist
		seq_printf(buf, "0\n");
	}
	return 0;
}
...
void static create_chargerIC_status_proc_file(void)
{
	...
	if (chargerIC_status_proc_file) {
		printk("[smb1351 Proc]%s sucessed!\n", __FUNCTION__);
	} else{
		printk("[smb1351 Proc]%s failed!\n", __FUNCTION__);
	}
}
#endif
//<ASUS alex wang 20160426>support chargeIC status----

static int otg_toggle(struct smb1351_charger *chip,bool toggle)
{
	int ret;

	if (!chip) {
		pr_info("Warning: smb1351_charger_dev is null due to probe function has error\n");
		return -1;
	}
	...
	if(toggle)
	{
		smb1351_masked_write(smb1351_charger_dev,OTG_USBIN_AICL_CTRL_REG,OTG_ID_PIN_CTRL_MASK,0x0);
		ret = smb1351_chg_otg_regulator_enable_asus(smb1351_charger_dev);
		if (ret < 0)
		{
			printk("smb1351 otg enable faill \n");
			return ret;
		}
		printk("smb1351 otg enable \n");
	}
	else
	{
		ret = smb1351_chg_otg_regulator_disable_asus(smb1351_charger_dev);
		if (ret < 0)
		{
			printk("smb1351 otg disable faill \n");
			return ret;
		}
		printk("smb1351 otg disable \n");
	}
	return 0;
}


int smb1351_asus_set_otg(bool toggle)
{
	int ret;
	if (!smb1351_charger_dev) {
		pr_info("Warning: smb1351_charger_dev is null due to probe function has error\n");
		return -1;
	}
	smb1351_enable_volatile_writes(smb1351_charger_dev);
	//smb1351_masked_write(smb1351_charger_dev,OTG_USBIN_AICL_CTRL_REG,OTG_ID_PIN_CTRL_MASK,0x80);
	if(toggle)
	{
		ret = otg_toggle(smb1351_charger_dev,true);
		if(ret < 0)
		{
		...
		//1000mA
		ret = smb1351_masked_write(smb1351_charger_dev,OTG_TLIM_CTRL_REG,OTG_OC_LIMIT_MASK,0xc);
		if (ret< 0) {
	...
	{
		ret = otg_toggle(smb1351_charger_dev,false);
		if(ret < 0)
		{
		...
		//500mA
		ret = smb1351_masked_write(smb1351_charger_dev,OTG_TLIM_CTRL_REG,OTG_OC_LIMIT_MASK,0x4);
		if (ret< 0) {
			pr_err("Couldn't configure set_OTG_ICFG ret=%d\n", ret);
			return ret;
		}
...
EXPORT_SYMBOL_GPL(smb1351_asus_set_otg);

static void asus_otg_init_zd552kl(struct smb1351_charger *chip)
{
	u8 reg; 
	smb1351_enable_volatile_writes(chip);
	smb1351_masked_write(chip,OTG_TLIM_CTRL_REG, OTG_OC_LIMIT_MASK, 0x0);
	smb1351_masked_write(chip,OTG_USBIN_AICL_CTRL_REG,OTG_ID_PIN_CTRL_MASK,0x80);
	smb1351_read_reg(chip,OTG_USBIN_AICL_CTRL_REG,&reg);
	printk("asus_otg_init_zd552kl 0x09=%x\n",reg);
}

void asus_show_smb_info(void)
{
	u8 reg[3];

	smb1351_read_reg(smb1351_charger_dev,CHG_CURRENT_CTRL_REG,&reg[0]);
	smb1351_read_reg(smb1351_charger_dev,CMD_INPUT_LIMIT_REG,&reg[1]);
	smb1351_read_reg(smb1351_charger_dev,CHG_PIN_EN_CTRL_REG,&reg[2]);
	printk("[BATT][SMB]fcc-icl[01] =0x%x, suspend =%s, batt_en[06] = 0x%x\n",
		reg[0], 
		(reg[1] &0x40) ? "yes" : "no",
		reg[2]);	
}

int smb1351_asus_set_usb_suspend(bool enable)
{
	int rc = -1;

	if (!smb1351_charger_dev) {
		pr_err("smb1351_charger_dev is null,will return!\n");
		return -1;
	}

	if(smb1351_charger_dev->parallel_charger_present){
		rc = smb1351_usb_suspend(smb1351_charger_dev, DEMO_APP, enable);
		if (rc)
			pr_err("DEMO_APP suspend charger failed, enable=%d\n", enable);
	}
