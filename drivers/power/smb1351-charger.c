						...
						int present)
{
	...
	if (present) {
		...
		//set HC mode
		rc = smb1351_masked_write(chip,
				CMD_INPUT_LIMIT_REG, 0x9, 0x9);
		if (rc) {
			pr_err("Couldn't set hc mode rc=%d\n", rc);
			return rc;
		}
		//disable aicl
		rc = smb1351_masked_write(chip,
				VARIOUS_FUNC_REG, 0x10, 0x0);
		if (rc) {
		...
		//set watdog
		rc = smb1351_masked_write(chip,
				WDOG_SAFETY_TIMER_CTRL_REG, 0x0, 0x1);
		if (rc) {
			pr_err("Couldn't disable aicl rc=%d\n", rc);
			return rc;
		}
		...
		//jeita setting
		rc = smb1351_masked_write(chip,
				HARD_SOFT_LIMIT_CELL_TEMP_REG, 0x30, 0x30);
		if (rc) {
			pr_err("Couldn't disable aicl rc=%d\n", rc);
			return rc;
		}
		rc = smb1351_masked_write(chip,
				THERM_A_CTRL_REG, 0xf, 0x0);
		if (rc) {
			pr_err("Couldn't disable aicl rc=%d\n", rc);
			return rc;
		}
