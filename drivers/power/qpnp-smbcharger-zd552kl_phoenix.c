...
static int g_vbus_low_impedance_detected = -1;/* ASUS_BSP steven1_wang: Add vbus lid support */
static int  COUNT = 0;// ASUS_BSP Freeman +++
...
#define asus_adaptor_detect_soc 1
...
struct smbchg_chip {
	...
	int vbus_low_impedance_irq;/* ASUS_BSP steven1_wang: Add vbus lid support */
	struct delayed_work vbus_low_impedance_det_work;
};
...
#else
static int smbchg_main_chg_fcc_percent = 100;
#endif
module_param_named(
...
#else
static int smbchg_main_chg_icl_percent = 100;
#endif
module_param_named(
...
static struct power_supply *get_parallel_psy(struct smbchg_chip *chip)
{
	...
	if (!chip->parallel.psy)
		pr_smb(PR_REGISTER, "parallel charger not found\n");
	return chip->parallel.psy;
}
...
//<ASUS-BSP Robert_He 20170418> add usb detect for touch charger mode  ++++++
extern void touch_usb_detection(bool plugin);
//<ASUS-BSP Robert_He 20170418> add usb detect for touch charger mode  ------
...
//<ASUS-BSP Robert_He 20170418> add usb detect for touch charger mode  ++++++
	if(status == POWER_SUPPLY_STATUS_CHARGING)
		touch_usb_detection(1);
	else
		touch_usb_detection(0);
//<ASUS-BSP Robert_He 20170418> add usb detect for touch charger mode  ------
	smbchg_vfloat_adjust_check(chip);
...
static void handle_usb_insertion(struct smbchg_chip *chip)
{
	...
	//schedule_delayed_work(&chip->thermal_policy_work,60*HZ);
...
static int asus_is_type_should_thermal_policy(struct smbchg_chip *chip)
{
	...
	if((chip->dual_charge == TYPEC_3A)|| (chip->dual_charge == ASUS_2A) )
		x = true;
...
static int smbchg_judge_thermal_level(struct smbchg_chip *chip,int temp,bool on){

	int level,pre_level;

	pre_level = chip->therm_lvl_sel;
		
	pr_debug("[US5587]panel %s,temp =%d\n",on ? "on":"off",temp);
	...
	pr_debug("[US5587]thermal level now is %d,pre is %d\n",level,pre_level);
	return level;
}

static void asus_thermal_policy_work(struct work_struct *work)
{
	struct smbchg_chip *chip= container_of(work, struct smbchg_chip, thermal_policy_work.work);

	struct qpnp_vadc_result thermal_result;
	int temp,thermal_levl=0;
	int ret;
	ret = qpnp_vadc_read(chip->thermal_vadc,P_MUX1_1_1,&thermal_result);
	if (ret)
		printk("[CHARGE]thermal_policy could not get adc value");

	temp = thermal_result.physical;
	thermal_levl = smbchg_judge_thermal_level(chip,temp,0);

	smbchg_system_temp_level_set(chip,thermal_levl);
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	rc = smbchg_sec_masked_write(chip,
				...
				0x8, 0x0);
	if (rc < 0) {
			pr_err("Couldn't disable hvdcp config in chgpath_chg rc=%d\n",
					rc);
			return rc;
	}
...
#define        OTG_DCP_FILE "driver/otgdcp"
static int otg_dcp_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n",otgdcp);
	return 0;
}
...
{
	return single_open(file, otg_dcp_read, NULL);
}
static const struct file_operations otg_dcp_fops = {
	.owner = THIS_MODULE,
	.open =  otg_dcp_open,
	.write = otg_dcp_write,
	.read = seq_read,
...
static void create_asus_otg_dcp_proc_file(void)
{
	struct proc_dir_entry *otg_dcp_entry = proc_create(OTG_DCP_FILE, 0777, NULL, &otg_dcp_fops);
	if (!otg_dcp_entry) printk("[smbchg]%s failed!\n",OTG_DCP_FILE);
}



/* +++ ASUS_BSP steven1_wang: Add vbus lid support +++*/
#define	VBUS_LID_FILE "driver/vbus_lid"
static int asus_vbus_lid_read(struct seq_file *buf, void *v)
{
	seq_printf(buf, "%d\n", g_vbus_low_impedance_detected);
	return 0;
}
static ssize_t asus_vbus_lid_write(struct file *filp, const char __user *buff, size_t len, loff_t *data)
{
	char messages[256];

	if(len>256) len=256;
	if(!smb_charger_dev){
		pr_err("smb_charger_dev is NULL!\n");
		g_vbus_low_impedance_detected = -1;
		return len;
	}

	if (copy_from_user(messages, buff, len)) return -EFAULT;

	g_vbus_low_impedance_detected = gpio_get_value(smb_charger_dev->vbus_low_impedance_irq);

	return len;
}
static int asus_vbus_lid_open(struct inode *inode, struct  file *file)
{
	return single_open(file, asus_vbus_lid_read, NULL);
}
static const struct file_operations asus_vbus_lid_fops = {
	.owner = THIS_MODULE,
	.open =  asus_vbus_lid_open,
	.write = asus_vbus_lid_write,
	.read = seq_read,
	.release = single_release,
};
static void create_asus_vbus_low_impedance_detection_proc_file(void)
{
	struct proc_dir_entry *vbus_lid_entry = proc_create(VBUS_LID_FILE, 0777, NULL, &asus_vbus_lid_fops);
	if (!vbus_lid_entry) printk("[smbchg]%s failed!\n",VBUS_LID_FILE);
}
/* --- ASUS_BSP steven1_wang: Add vbus lid support ---*/
...
#define PCC_200MA 		BIT(1)
#define PCC_150MA 		BIT(0)
#define FCC_1000MA 		1000
#define FCC_1200MA 		1200
#define FCC_700MA       700
#define ASUS_FlOAT_4V34 		4340
#define ASUS_FlOAT_4V35 		4350
#define ASUS_FlOAT_4V38 		4380
#define ASUS_FlOAT_4V06 		4060
#define ASUS_FlOAT_4V01 		4010
#define RCHG_100MV 		0
#define RCHG_200MV 		1
#define TCC_150MA 150
#define TCC_200MA 200

const int sr_jeita_cfg[]={15,100,200,500,600};
const int sr_jeita_rchg_cfg[]={45,130,230,470,570};
...
static void asus_charge_config_sr(struct smbchg_chip *chip){
	...
	chip->asus_tb.pcc_cfg=PCC_200MA;
...
static void asus_soft_jeita_config_ze553kl(struct smbchg_chip *chip,bool *charging_enable,int *fcc_value, int *float_volt)
{
	...
	switch (chip->soft_jeita_state) {
		case JEITA_STATE_RANGE_01:
			...
			*fcc_value = 700;	//chip->asus_tb.jeita_fcc_cfg[0];
		...
		case JEITA_STATE_RANGE_03:
			*charging_enable = true;
			*fcc_value = 1200;	//chip->asus_tb.jeita_fcc_cfg[1];
			*float_volt = chip->asus_tb.fv_cfg;
		...
		case JEITA_STATE_RANGE_04:
			if(batt_volt>4200){
				*fcc_value = 900;	//chip->asus_tb.jeita_fcc_cfg[0];
			}else if(batt_volt<=4000){
				*fcc_value = 2600;	//chip->asus_tb.fcc_cfg;
			}else{
				*fcc_value = 1880;	//chip->asus_tb.fcc_cfg;
			}
			*charging_enable = true;
			*float_volt = chip->asus_tb.fv_cfg;
		...
		case JEITA_STATE_RANGE_05:
			if((batt_volt>=4060)&&(batt_float_volt==chip->asus_tb.fv_cfg)){
				*charging_enable = false;
				*float_volt = chip->asus_tb.fv_cfg;
...
static void asus_typec_dfp_setting_work_3A(struct work_struct *work)
{
		...
		if((aicl_done ==1) && (aicl_result <=1500)){
			chip->dual_charge = SINGLE;
			read_usb_type(chip, &usb_type_name, &usb_supply_type);
			if(usb_supply_type==POWER_SUPPLY_TYPE_USB){
				rc = vote(chip->usb_icl_votable,PSY_ICL_VOTER, true, 500);
...
#ifdef ASUS_FACTORY_BUILD
			schedule_delayed_work(&chip->asus_adapter_det_work,msecs_to_jiffies(ASUS_DETECT_MS_3S));
#else
...
static void asus_adapter_detect_reset_work(struct smbchg_chip *chip)
{
    ...
    if(chip->dual_charge==ASUS_2A){
        ...
        asus_change_usbin(chip,1910);
    }else if(chip->BR_countrycode_flag==0){
        chip->asus_charging_type=DCP_OTHERS_1A;
    }
	return;
}
...
void asus_read_BR_countrycode_work(struct work_struct *work)
{
	...
	buf[readlen] = '\0';
	printk("[BAT][CHG] country code = %s readlen = %d\n", buf, readlen);
	//if (strcmp(buf, "BR") == 0){
	if((buf[0]=='B') && (buf[1]=='R')){
...
static void smbchg_pre_config_ze553kl(struct smbchg_chip *chip)
{
	...
	rc = smbchg_sec_masked_write(chip, chip->usb_chgpth_base + 0x40,
			BIT(3),BIT(3));
	if (rc < 0) {
		dev_err(chip->dev, "Couldn't set DCIN_SUSPEND rc=%d\n", rc);
	}
...
/* +++ ASUS_BSP steven1_wang: Add vbus lid support +++*/
static irqreturn_t vbus_low_impedance_irq(int irq, void *_chip)
{
    struct smbchg_chip *chip = _chip;

	g_vbus_low_impedance_detected = 2;
	schedule_delayed_work(&chip->vbus_low_impedance_det_work, msecs_to_jiffies(500));

	return IRQ_HANDLED;
}

static bool liquid_flag = 0;
static ssize_t vbus_liquid_switch_state(struct switch_dev *sdev, char *buf)
{
	return sprintf(buf, "%d\n", liquid_flag);
}

void asus_vbus_low_impedance_det_work( struct work_struct *work)
{
	struct smbchg_chip *chip = container_of(work,
				struct smbchg_chip,
				vbus_low_impedance_det_work.work);
    int value = 0;
	int otg_enable = 0;

	otg_enable = smbchg_otg_regulator_is_enable(chip->otg_vreg.rdev);
	if(otg_enable){
		liquid_flag = 0;
		goto DONE;
	}

    value = gpio_get_value(chip->vbus_low_impedance_irq);
    if(value){
		liquid_flag = 1;
		// power off
		vote(chip->usb_suspend_votable, LIQUID_EN_VOTER, 1, 0);
		COUNT ++;
		ASUSEvtlog("Low Impedance OR Water Seepage! Time:%d",COUNT);
	}else{
		liquid_flag = 0;
		//power on
		vote(chip->usb_suspend_votable, LIQUID_EN_VOTER, 0, 0);
		switch_set_state(&chip->vbus_liquid, liquid_flag);
	}

	//report uevent
	if(liquid_flag && chip->usb_present){
		switch_set_state(&chip->vbus_liquid, liquid_flag);
	}

DONE:
    pr_info_ratelimited("otg_enable=%d;pin status =%d;liquid_flag=%d;usb_present=%d\n",
			otg_enable, value, liquid_flag, chip->usb_present);
    ASUSErclog(ASUS_VBUS_LOW_IMPEDANCE, "USB LID  is triggered");
}
/* --- ASUS_BSP steven1_wang: Add vbus lid support ---*/
...
static int smbchg_request_asus_adapter_gpio(struct smbchg_chip *chip)
{
	...
	rc = gpio_request(chip->dcp_chg_en,"dcp_chg_en");
	if(rc){
		dev_err(&chip->spmi->dev,"dcp_chg_en request failed!\n");
		return -EINVAL;
	}

	gpio_direction_output(chip->dcp_chg_en,0);
	if(rc){
		dev_err(&chip->spmi->dev,"dcp_chg_en request failed!\n");
		return -EINVAL;
	}
...
/* +++ ASUS_BSP steven1_wang: Add vbus lid support +++*/
	chip->vbus_low_impedance_irq=of_get_named_gpio(node,"vbus-low-impedance-irq",0);
	pr_smb(PR_STATUS,"vbus-low-impedance-irq=%d\n",chip->vbus_low_impedance_irq);

	if(!gpio_is_valid(chip->vbus_low_impedance_irq)){
		dev_err(&chip->spmi->dev,"vbus-low-impedance-irq is not valid!\n");
		return -EINVAL;
	}

	rc = gpio_request(chip->vbus_low_impedance_irq,"vbus-low-impedance-irq");
	if(rc){
		dev_err(&chip->spmi->dev,"vbus-low-impedance-irq request failed!\n");
		return -EINVAL;
	}

	rc = gpio_direction_input(chip->vbus_low_impedance_irq);
	if (rc < 0) {
		pr_err("gpio_direction_input vbus-low-impedance-irq failed, status = %d \n", rc);
		gpio_free(chip->vbus_low_impedance_irq);
		return -EINVAL;
	}

	irq_num = gpio_to_irq(chip->vbus_low_impedance_irq);
	if (irq_num < 0) {
		pr_err("vbus-low-impedance-irq gpio_to_irq failed\n");
		gpio_free(chip->vbus_low_impedance_irq);
		return -EINVAL;
	}

	if (request_irq(irq_num, vbus_low_impedance_irq, IRQF_TRIGGER_RISING|IRQF_TRIGGER_FALLING,
			"vbus_low_impedance_irq", chip) < 0) {
		pr_err("request_irq vbus-low-impedance-irq failed \n");
		return -EINVAL;
	}
/* --- ASUS_BSP steven1_wang: Add vbus lid support ---*/
...
static int asus_print_all(void)
{
	...
	ASUSEvtlog("[BAT][Ser]report Capacity==>%d,FCC:%d,BMS:%d,V:%d,Cur:%d,Temp:%s%d.%dC,Cable:%d(%s),Status:%s\n", 
		...
		batt_status_str[batt_info.status]);
#ifdef ASUS_FACTORY_BUILD
	printk("[BATT][FAC]charger_batt_enable =%d chg_limit_charge_flag=%d\n",charger_batt_enable,chg_limit_charge_flag);
#endif
...
static int smbchg_probe(struct spmi_device *spmi)
{
	...
	INIT_DELAYED_WORK(&chip->vbus_low_impedance_det_work, asus_vbus_low_impedance_det_work);
	...
	//register switch device for liquid status
	chip->vbus_liquid.name = "vbus_liquid";
	chip->vbus_liquid.print_state = vbus_liquid_switch_state;
	if(switch_dev_register(&chip->vbus_liquid) < 0) {
		pr_err("register liquid switch faild!\n");
		goto out;
	}
...
//<ASUS-alexwang20160309-2>support asus factory battery voltage and current----
	...
	chip->thermal_vadc = qpnp_get_vadc(chip->dev,"thermal");
	if(chip->thermal_vadc ==NULL){
...
unregister_alert_regulator:
		if(chip->usb_alert_reg)
				regulator_set_voltage(chip->usb_alert_reg,0, 3000000);
		return rc;
