...
static int get_prop_capacity(struct fg_chip *chip)
{
	...
	bool vbatt_low_sts;
	static int old_msoc = -1 ,count = 0, chase_msoc = -1;
...
static int get_sram_prop_now(struct fg_chip *chip, unsigned int type)
{
	...
	if ((type == FG_DATA_BATT_TEMP) && (g_fake_battery_temperature == 690)){
		return 610;
	}
...
{
	if(!chip_oem){
		pr_err("chip is NULL,will return!\n");
		return ;
	}
	set_prop_jeita_temp(chip_oem, FG_MEM_HARD_HOT, value);
}
...
static ssize_t batt_switch_name(struct switch_dev *sdev, char *buf)
{
	...
	const char* bat_version= "10.30.0.28";

	return sprintf(buf, "%s%s%s%s%s\n", bat_model, bat_cell, bat_id, bat_profile, bat_version);
}
