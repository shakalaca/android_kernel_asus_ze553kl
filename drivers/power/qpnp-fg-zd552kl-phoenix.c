...
static struct fg_mem_setting settings[FG_MEM_SETTING_MAX] = {
	...
	SETTING(HARD_COLD,       0x454,   2,      0),// ASUS_BSP steven1_wang: porting fg_mem_setting for ZD552KL
	SETTING(HARD_HOT,        0x454,   3,      720),// ASUS_BSP steven1_wang: porting fg_mem_setting for ZD552KL
...
struct fg_chip {
	...
	bool			fg_shutdown;
	struct delayed_work	optimize_full_work;
...
static char EnUsoc_DisUsoc[2][256] = { \
	{0,0,1,1,2,2,2,3,3,4,4,4,5,5,6,6,6,7,7,8,8,8,9,9,10,10,10,11,11,12,12,12,13,13,13,14,14,15,15,15,16,16,17,17,17,18,18, \
	 19,19,19,20,20,21,21,21,22,22,23,23,23,24,24,25,25,25,26,26,27,27,27,28,28,29,29,29,30,30,31,31,31,32,32,33,33,33, \
	 34,34,35,35,35,36,36,37,37,37,38,38,38,39,39,40,40,40,41,41,42,42,42,43,43,44,44,44,45,45,46,46,46,47,47,48,48,48, \
     49,49,50,50,50,51,51,52,52,52,53,53,54,54,54,55,55,56,56,56,57,57,58,58,58,59,59,60,60,60,61,61,62,62,62,63,63,63, \
     64,64,65,65,65,66,66,67,67,67,68,68,69,69,69,70,70,71,71,71,72,72,73,73,73,74,74,75,75,75,76,76,77,77,77,78,78,79, \
     79,79,80,80,81,81,81,82,82,83,83,83,84,84,85,85,85,86,86,87,87,87,88,88,88,89,89,90,90,90,91,91,92,92,92,93,93,94, \
     94,94,95,95,96,96,96,97,97,98,98,98,99,99,100,100,100,100,100}, \
	{0,0,1,1,2,2,2,3,3,4,4,4,5,5,6,6,6,7,7,8,8,8,9,9,10,10,10,11,11,12,12,12,13,13,14,14,14,15,15,16,16,16,17,17,18,18,18, \
	 19,19,20,20,20,21,21,22,22,22,23,23,24,24,24,25,25,26,26,26,27,27,28,28,28,29,29,30,30,30,31,31,32,32,32,33,33,34, \
	 34,34,35,35,36,36,36,37,37,38,38,38,39,39,40,40,40,41,41,42,42,42,43,43,44,44,44,45,45,46,46,46,47,47,48,48,48,49, \
	 49,50,50,50,51,51,51,52,52,53,53,53,54,54,55,55,55,56,56,57,57,57,58,58,59,59,59,60,60,61,61,61,62,62,63,63,63,64, \
	 64,65,65,65,66,66,67,67,67,68,68,69,69,69,70,70,71,71,71,72,72,73,73,73,74,74,75,75,75,76,76,77,77,77,78,78,79,79, \
	 79,80,80,81,81,81,82,82,83,83,83,84,84,85,85,85,86,86,87,87,87,88,88,89,89,89,90,90,91,91,91,92,92,93,93,93,94,94, \
	 95,95,95,96,96,97,97,97,98,98,99,99,100,100,100,100,100,100,100}, \
};

static int  filedata = 0, FinalUsoc;
static bool ChargingOrDischargingRote = DisCharging ,InitReadUsoc = 0;
static int Usoc_mapping(int old_msoc,int msoc)
{
	static int Usoc = -1, DischargingJumpBackup ,ChargingJumpBackup, dischargingrote = 0 ,chargingrote = 0;
	/*
		Note: As battery team mapping table. at the same msoc, delta (abs(charging usoc-discharging usoc)) awalys <=1%,
		so usoc jump more than 1% will not happen.
		Charging usoc awalys <= discharging usoc.
	*/
	/*
	if(Usoc == -1){
		if(FirstUsocMapping <= 0) // load new ocv table or other situation(...)
			FirstUsocMapping = EnUsoc_DisUsoc[DisCharging][msoc];
		Usoc = FirstUsocMapping;
		FinalUsoc = Usoc;
		pr_info("old_msoc:%d ,msoc:%d ,rsoc:%d ,Usoc:%d\n",old_msoc,msoc,(DIV_ROUND_CLOSEST((msoc - 1) * 98, 253) + 1),Usoc);
		return Usoc;
	}*/
	pr_info("ChargingOrDischargingRote:%d DisCharging:%d Charging:%d ChargingJumpBackup:%d chargingrote:%d DischargingJumpBackup:%d dischargingrote:%d\n", \
	ChargingOrDischargingRote,EnUsoc_DisUsoc[DisCharging][msoc],EnUsoc_DisUsoc[Charging][msoc], \
	ChargingJumpBackup,chargingrote,DischargingJumpBackup,dischargingrote);

	if(ChargingOrDischargingRote){
		if(old_msoc > msoc){//also Discharging
			Usoc = EnUsoc_DisUsoc[DisCharging][msoc];
			chargingrote = 0;
		}
		else{
			if( chargingrote == 1){
				if( ChargingJumpBackup > EnUsoc_DisUsoc[Charging][msoc])
					return ChargingJumpBackup;
				else{
					ChargingOrDischargingRote = Charging;
					Usoc = EnUsoc_DisUsoc[Charging][msoc];
					chargingrote = 0;
				}
			}
			//first discharging jump to charging, if discharging level greater than charging level, should report discharging level
			if( (EnUsoc_DisUsoc[DisCharging][old_msoc] > EnUsoc_DisUsoc[Charging][msoc]) && (chargingrote == 0)){
				Usoc = EnUsoc_DisUsoc[DisCharging][old_msoc];
				ChargingJumpBackup = Usoc;
				chargingrote = 1;
				return Usoc;
			}else{
				ChargingOrDischargingRote = Charging;
		...
		if(old_msoc < msoc){//also Charging
			Usoc = EnUsoc_DisUsoc[Charging][msoc];
			dischargingrote = 0;
		}else{// Charging to discharging
			if( dischargingrote == 1){
				if( DischargingJumpBackup > EnUsoc_DisUsoc[DisCharging][msoc])
					return DischargingJumpBackup;
				else{
					ChargingOrDischargingRote = DisCharging;
					Usoc = EnUsoc_DisUsoc[DisCharging][msoc];
					dischargingrote = 0;
				}
			}
			//first charging jump to discharging, if discharging level greater than charging level, should report charging level
			if( (EnUsoc_DisUsoc[DisCharging][msoc] > EnUsoc_DisUsoc[Charging][old_msoc]) && (dischargingrote == 0)){
				Usoc = EnUsoc_DisUsoc[Charging][old_msoc];
				DischargingJumpBackup = Usoc;
				dischargingrote = 1;
				return Usoc;
			}else{
				ChargingOrDischargingRote = DisCharging;
...
static int init_asus_capacity(void)
{
	...
	if (fp->f_op != NULL && fp->f_op->read != NULL) {
		...
		pr_info("%s: f_op=NULL or op->read=NULL\n", __func__);
...
static int backup_asus_capacity(int input)
{
	struct file *fp = NULL;
	mm_segment_t old_fs;
	loff_t pos_lsts = 0;
	...
	if (fp->f_op != NULL && fp->f_op->write != NULL) {
		...
		pr_info("%s: f_op=NULL or op->write=NULL\n", __func__);
		return -2;
	}
	set_fs(old_fs);
	filp_close(fp, NULL);
...
void init_asus_capacity_worker(struct work_struct *work)
{
	struct fg_chip *chip = container_of(work,
				struct fg_chip,
				init_asus_capacity_work.work);

	pr_info(" %d \n",init_asus_capacity());
	schedule_delayed_work(&chip->init_asus_capacity_work, HZ * 5);
}
#endif
...
static int soc_raw_optimize(struct fg_chip *chip)
{
	int i,msoc[3]={-1,-1,-1};
	int optimize_soc=0,remainder;
	static int backup_msoc = -1;
	
	for(i=0;i<3;i++){
		msoc[i] = get_monotonic_soc_raw(chip);
		//msleep(1);
		optimize_soc += msoc[i];
	}

	remainder = optimize_soc%3;
	if(remainder != 0){
		if(backup_msoc == -1){
			backup_msoc = msoc[2];
			pr_info("first power on backup_msoc:%d\n",backup_msoc);
			return msoc[2];
		}
		pr_info("discard current data msoc:%d %d %d\n",msoc[0],msoc[1],msoc[2]);
		if (chip->batt_psy)
			power_supply_changed(chip->batt_psy);
		else
			pr_info("update_usoc_work power_supply_changed fail\n");
		return backup_msoc;
	}
	else{
		backup_msoc = msoc[0];
...
bool optimize_full_work_complete = 0;
static void optimize_full_work(struct work_struct *work)
{
	optimize_full_work_complete = 1;
	if (chip_oem->batt_psy)
		power_supply_changed(chip_oem->batt_psy);
	pr_info("optimize_full_work_complete \n");
}

static int get_prop_capacity(struct fg_chip *chip)
{
	int msoc, rc , msoc_raw ,time ,current_now;
	bool vbatt_low_sts;
	static int old_msoc = -1 ,count = 0, chase_msoc = -1, Usoc, Usoc_info;
	static bool charge_full_status = 0, optimize_full_work_start = 0 ;
	...
	if((charge_full_status == 1) && (!chip->charge_full)){
		charge_full_status = 0;
		msoc = soc_raw_optimize(chip);
		pr_info("optimize_full_work_start  msoc:%d\n",msoc);
		if(msoc != FULL_SOC_RAW){
			optimize_full_work_start = 1;
			current_now = get_sram_prop_now(chip, FG_DATA_CURRENT);
			if(current_now < 0)
			...
			if(current_now <= 2000)
				...
				time = 30000;
			cancel_delayed_work_sync(&chip->optimize_full_work);
			schedule_delayed_work(&chip->optimize_full_work,msecs_to_jiffies(time));
			return FULL_CAPACITY;
		}
	}

	if(optimize_full_work_start){
		if(optimize_full_work_complete){
			optimize_full_work_start = 0;
			optimize_full_work_complete = 0;	
			msoc_raw = soc_raw_optimize(chip);
			msoc = DIV_ROUND_CLOSEST((msoc_raw - 1) * (FULL_CAPACITY - 2),FULL_SOC_RAW - 2) + 1;
			pr_info("optimize_full_work_complete end count:%d msoc:%d msoc_raw:%d\n",count,msoc,msoc_raw);	
			
			if (chip_oem->batt_psy)
					power_supply_changed(chip_oem->batt_psy);
			
			if( (FULL_CAPACITY - count - msoc) >= 2){
				optimize_full_work_start = 1;
				current_now = get_sram_prop_now(chip, FG_DATA_CURRENT);
				if(current_now < 0)
					current_now = -(current_now /1000);
				...
				if(msoc_raw == FULL_SOC_RAW){
					pr_info("chase_msoc full\n");
					return FULL_CAPACITY;
				}
				else{
					chase_msoc = msoc;
					pr_info("chase_msoc:%d end\n",chase_msoc);					
					return chase_msoc;
				}
			}
			count ++;
			chase_msoc = (FULL_CAPACITY - count);
			pr_info("chase_msoc:%d\n",chase_msoc);
			return chase_msoc;
		}	
		else{
			if(chase_msoc != -1)
	...
	if (chip->charge_full){
		/*when the heavy loading let the battery discharging, don't keep 100% any more*/
		if(soc_raw_optimize(chip) <= 244){
			chip->charge_full = 0;	
	...
	if(old_msoc == -1){
		if(InitReadUsoc == 1){
			old_msoc = (filedata >> 1);
			filedata = ( (msoc << 1) + ChargingOrDischargingRote);
		}else
			old_msoc = soc_raw_optimize(chip) + 1;// +1,when gaugeMappingBackup not created, should backup date,need run "if(old_msoc != msoc)"
		pr_info("mosc first change old_msoc:%d ,InitReadUsoc:%d\n",old_msoc,InitReadUsoc);
	}
	...
	if(old_msoc != msoc)
	{
		Usoc = Usoc_mapping(old_msoc,msoc);
		pr_info("old_msoc:%d ,msoc:%d ,rsoc:%d ,chip->status:%d Usoc:%d ChargingOrDischargingRote:%s other:%d %d\n", \
			old_msoc,msoc,(DIV_ROUND_CLOSEST((msoc - 1) * 98, 253) + 1),chip->status, \
			Usoc,ChargingOrDischargingRote?"DisCharging":"Charging", \
			(msoc << 1),ChargingOrDischargingRote);
		Usoc_info = ( (msoc << 1) + ChargingOrDischargingRote );
		rc = backup_asus_capacity(Usoc_info);
		if(rc < 0){
			msleep(100);
			pr_info("backup asus capacity fail , retry one more time\n");
			backup_asus_capacity(Usoc_info);
		}
		FinalUsoc = Usoc;
		if (chip->batt_psy)
			power_supply_changed(chip->batt_psy);
	...
	pr_info_ratelimited("Rsoc:%d Usoc:%d\n",(DIV_ROUND_CLOSEST((msoc - 1) * 98, 253) + 1),FinalUsoc);
	if( (FinalUsoc >= 0) && (FinalUsoc <= 100) )
		return FinalUsoc;
	else
...
int extern_get_capacity(void){
	if(chip_oem != NULL)
		return get_monotonic_soc_raw(chip_oem);
	else
		pr_info("extern_get_capacity chip_oem = NULL");
	return 0;
}
...
extern int g_fake_battery_temperature;
...
static int get_sram_prop_now(struct fg_chip *chip, unsigned int type)
{
	...
	if (type == FG_DATA_CURRENT)
	{		
		if((fg_data[type].value <= 5000) && (fg_data[type].value >= -5000)){
			pr_info("fake current :%d\n",fg_data[type].value);
			return 0;
		}
	}	   
...
#define BATT_OVP_THRESH_MV	4400
	...
	for (i = 1; i < FG_DATA_MAX; i++) {
		...
		case FG_DATA_CPRED_VOLTAGE:
			fg_data[i].value = div_u64(
                ...
                (fg_data[i].value/1000) > BATT_OVP_THRESH_MV) {
...
wait:
	...
	pr_err("BATT_TEMP %d %d\n", temp, fg_data[0].value);     //print every 10s
...
/* ASUS_BSP steven1_wang: Calculate and store battery cycle count+++ */
#define CYCLE_FULL_THRESH_DEFAULT   85 // 85% as one full cycle
...
#define CYCLE_COUNT_FILE_NAME   "/dev/block/platform/soc/7824900.sdhci/by-name/fsc_old"
#define FILE_OP_READ   0
#define FILE_OP_WRITE   1
#define CYCLE_COUNT_DATA_MAGIC  0x85
#define CYCLE_COUNT_DATA_OFFSET  0x00
...
static int g_cyclecount_initialized = 0;
...
static int init_batt_cycle_count(void)
{
	int i, rc = 0;
...
static int asus_update_cycle_count(struct fg_chip *chip)
{
	...
	cur_soc[0] = reg[2];
	/* System Soc */
	rc = fg_mem_read(chip, reg, CYCLE_SYSTEM_SOC_REG, 2,
			CYCLE_SYSTEM_SOC_OFFSET, 0);
	if (rc) {
	...
	cur_soc[1] = reg[1];
	/* Monotonic Soc */
	rc = fg_mem_read(chip, reg, CYCLE_MONO_SOC_REG, 2,
			CYCLE_MONO_SOC_OFFSET, 0);
	if (rc) {
	...
	if(!g_cyclecount_initialized) {
		init_batt_cycle_count();/* Try one more time */
	}
...
static irqreturn_t fg_soc_irq_handler(int irq, void *_chip)
{
	...
	//if (fg_debug_mask & FG_IRQS)
		pr_info("triggered 0x%x  soc:%d\n", soc_rt_sts,get_monotonic_soc_raw(chip));
...
/* +++ ASUS_BSP steven1_wang: Add for UI battery version +++*/
#define ASUS_BATT_PACK_MAX_COUNT   2
#define ASUS_BATT_TYPE_STR_MAXLEN   50
#define ASUS_BATT_VERSION_STR_MAXLEN   ASUS_BATT_TYPE_STR_MAXLEN
static const char * g_battery_version = NULL;
/* qcom,battery-type in batt profile dtsi */
static const char asus_batt_type_str[ASUS_BATT_PACK_MAX_COUNT][ASUS_BATT_TYPE_STR_MAXLEN] = {
    {"asus_phoenix_2900mah_10k"},/* LG */
    {"asus_phoenix_2900mah_100k"}/* Coslight */
};
/* UI Hardware Information -> Battery version */
static const char asus_batt_version_str[ASUS_BATT_PACK_MAX_COUNT][ASUS_BATT_VERSION_STR_MAXLEN] = {
    /* model - cell - id - profile - version */
    {"C11P1511-G-02-0001-3.1.10.55"},/* LG */
    {"C11P1511-O-03-0002-3.1.10.55"}/* Coslight */
};
/* --- ASUS_BSP steven1_wang: Add for UI battery version --- */
static int fg_batt_profile_init(struct fg_chip *chip)
{
	...
	u8 reg = 0;
    int i;
	static u8 ReadBackupCapacityCount = 0;
...
wait:
	...
	if (reg & PROFILE_INTEGRITY_BIT) {
		...
		filedata = init_asus_capacity();
		if(filedata <= 0){ //read file fail
			ReadBackupCapacityCount++;
			if(ReadBackupCapacityCount <= 15){ //delay 6*BATTERY_PSY_WAIT_MS
				pr_info("gauge backup file not ready, retray %d\n",ReadBackupCapacityCount);
				goto reschedule;
			}
			ReadBackupCapacityCount = 0;
			FinalUsoc = EnUsoc_DisUsoc[DisCharging][soc_raw_optimize(chip)];
		}else{
			pr_info("fist read gauge backup file ok\n");
			InitReadUsoc = 1;
			ChargingOrDischargingRote = (filedata&0x01);
			FinalUsoc = EnUsoc_DisUsoc[(filedata&0x01)][(filedata >> 1)];
		}
		//schedule_delayed_work(&chip->init_asus_capacity_work, HZ * 5);
		//if (fg_debug_mask & FG_STATUS)
		pr_info("Battery profiles same, using default last shutdown Usoc:%d filedata:%d other:%d %d\n", \
			FinalUsoc,filedata,(filedata >> 1),(filedata&0x01));
...
/* +++ ASUS_BSP steven1_wang: Add for UI battery version +++*/
    for(i = 0; i < ASUS_BATT_PACK_MAX_COUNT; i++) {
        if(0 == strncmp(batt_type_str, asus_batt_type_str[i], strlen(asus_batt_type_str[i]))) {
            g_battery_version = asus_batt_version_str[i];
            pr_info("Battery version: %s\n", g_battery_version);
            break;
        }
        if(i == ASUS_BATT_PACK_MAX_COUNT-1) {
            g_battery_version = NULL;
            pr_err("Couldn't find suitable battery version!\n");
        }
    }
/* --- ASUS_BSP steven1_wang: Add for UI battery version ---*/
...
//Add factory node for gauge status
static int gauge_proc_read(struct seq_file *buf, void *data)
{
	int rc;
	u8 fg_batt_sts;
	...
	rc = fg_read(chip_oem, &fg_batt_sts,
				 INT_RT_STS(chip_oem->batt_base), 1);
	if (rc) {
		printk("spmi read failed: addr=%03X, rc=%d\n",
				INT_RT_STS(chip_oem->batt_base), rc);
		rc = 0;
	}else
		rc = 1;
...
static int gauge_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, gauge_proc_read, NULL);
}
static const struct file_operations gauge_fops = {
	.owner = THIS_MODULE,
	.open = gauge_proc_open,
	.read = seq_read,
	.release = single_release,
};
static void create_gauge_proc_file(void)
{
...
/* ASUS_BSP steven1_wang: Add proc nodes for battery cycle count+++ */
...
static int cycle_count_proc_show(struct seq_file *buf, void *data)
{
	...
	(char *)&g_cycle_count_data, sizeof(struct CYCLE_COUNT_DATA), FILE_OP_WRITE);
	if(rc < 0 )
		printk("%s: write cycle count file error\n",__FUNCTION__);
	seq_printf(buf, "---Calculated cycle count---\n");
	seq_printf(buf, "Batt_Soc:%d.%d\n", cycle_count[0]/10, cycle_count[0]%10);
	seq_printf(buf, "Sys_Soc:%d.%d\n", cycle_count[1]/10, cycle_count[1]%10);
	seq_printf(buf, "Mono_Soc:%d.%d\n", cycle_count[2]/10, cycle_count[2]%10);
...
static void create_batt_cycle_count_proc_file(void)
{
	...
	struct proc_dir_entry *asus_batt_accum_soc_proc_file = proc_create("accum_soc", 0666,
		asus_batt_cycle_count_dir, &accum_soc_fops);
	struct proc_dir_entry *asus_batt_full_cycle_thresh_proc_file = proc_create("full_cycle_thresh", 0666,
...
/* ASUS_BSP steven1_wang: Write back battery cycle count before restart/shutdown +++ */
...
/*---ASUS_BSP Gauge Version Interface---*/
...
static int fg_probe(struct spmi_device *spmi)
{
	...
	create_gauge_proc_file();

	init_batt_cycle_count();
	...
	INIT_DELAYED_WORK(&chip->optimize_full_work, optimize_full_work);
//	INIT_DELAYED_WORK(&chip->init_asus_capacity_work, init_asus_capacity_worker);
