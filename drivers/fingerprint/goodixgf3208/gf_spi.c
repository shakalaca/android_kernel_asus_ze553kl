...
struct gf_key_map key_map[] =
{
      ...
      {  "DCLICK",  KEY_F20    },
      {  "LPRESS",  KEY_F21    },
      {  "ASUS_UP",    FRINGERPRINT_SWIPE_UP    },
      {  "ASUS_DOWN",  FRINGERPRINT_SWIPE_DOWN    },
      {  "ASUS_LEFT",  FRINGERPRINT_SWIPE_LEFT    },
      {  "ASUS_RIGHT", FRINGERPRINT_SWIPE_RIGHT    },
      {  "ASUS_TAP",   FRINGERPRINT_TAP    },
      {  "ASUS_DTAP",  FRINGERPRINT_DTAP    },
      {  "ASUS_PRESS", FRINGERPRINT_LONGPRESS    },
};
	...
	case GF_IOC_SETSPEED:
    	...
    	if (retval == 0) {
	    	if (speed > 12 * 1000 * 1000) {
		    	pr_debug("Set speed:%d is larger than 12Mbps.\n",	speed);
		    } else {
		   ...
		   pr_debug("Failed to get speed from user. retval = %d\n",	retval);
		}
#endif
		break;
	case GF_IOC_RESET:
		...
		if (copy_from_user
			...
			break;
                ...
                if(i == ARRAY_SIZE(key_map)) {
                    pr_debug("key %d not support yet \n", gf_key.key);
                    retval = -EFAULT;
                }
...
static int driver_init_partial(struct gf_dev *gf_dev)
{
	...
	if (!ret) {
		...
		//gf_enable_irq(gf_dev);
		gf_disable_irq(gf_dev);
		gf_dbg("irq request ok!%d\n",gf_dev->irq_requested);
	}
...
static void gf_reg_key_kernel(struct gf_dev *gf_dev)
{
    ...
    if (input_register_device(gf_dev->input))
        pr_err("Failed to register GF as input device.\n");
}
