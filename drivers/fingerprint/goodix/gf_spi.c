...
static int gfspi_ioctl_clk_enable(struct gf_dev *data)
{
	...
	pr_info("%s: enter. clk_enabled = %d\n", __func__, data->clk_enabled);
...
#ifdef AP_CONTROL_CLK
	    pr_info("spiclk:gfspi_ioctl_clk_enable.\n");
...
#ifdef AP_CONTROL_CLK
	    pr_info("spiclk:gfspi_ioctl_clk_disable.\n");
...
#endif
{
	...
	wake_lock_init(&gf_dev->wake_lock,WAKE_LOCK_SUSPEND,"FP_wake_lock");
