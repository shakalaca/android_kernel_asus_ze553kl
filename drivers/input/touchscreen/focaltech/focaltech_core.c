...
struct fts_NameList fts_ts_NameList = {//for touchscreen
	"fts_ts",
	"pmx_ts_active",
	"pmx_ts_suspend",
	"pmx_ts_release",
...
struct fts_NameList fts_cap_NameList = {//for virtual key
	"fts_cap",
	"pmx_virtual_active",
	"pmx_virtual_suspend",
	"pmx_virtual_release",
...
void asus_psensor_disable_virtualkey(bool enable)
{
	if(enable)
	{
		touch_proximity_at_phone = 1;//FOCALTECH_VIRTUALKEY_DISABLE = true;
		printk(KERN_INFO "[Touch] :proximity trigger disable virtual key !\n");
	} 
	else
	{
		touch_proximity_at_phone = 0;//FOCALTECH_VIRTUALKEY_DISABLE = false;
		printk(KERN_INFO "[Touch] : proximity trigger enable virtual key !\n");
	}
}
EXPORT_SYMBOL(asus_psensor_disable_virtualkey);
//<ASUS_Proximity->

u8 globe_fw_ver = 0;
u8 asus_get_capsensor_fw_ver(void)
{
	return globe_fw_ver;
}
EXPORT_SYMBOL(asus_get_capsensor_fw_ver);
...
//<ASUS-BSP Robert_He 20170405> add capsensor vkled function ------
	...
	memcpy( buf,data->buf_touch_data+(((data->buf_count_neg-1)%30)*POINT_READ_BUF), sizeof(u8)*POINT_READ_BUF );
...
/*
	...
	{
		printk("i=%d,event=%d,finger_id=%d,x=%d,y=%d,area=%d,pressure=%d\n",i,event->au8_touch_event[i],event->au8_finger_id[i],event->au16_x[i],event->au16_y[i],event->area[i],event->pressure[i]);
	}
*/
	if(data->IC_ID == ID_FT3267)	//report VirtualKey data
	{	
...
//<ASUS-BSP Robert_He 20170405> add capsensor vkled function ------				
	...
	else if(data->IC_ID == ID_FT3517)	//reprot touch data
	{
		for (i = 0; i < event->touch_point; i++)
		{
...
static void fts_touch_irq_work(struct work_struct *work)
{
	...
	struct fts_ts_data *fts_data =
			container_of(work, struct fts_ts_data, touch_event_work);

	ret = fts_read_Touchdata(fts_data);
	if (ret == 0)
		fts_report_value(fts_data);
	
	enable_irq(fts_data->client->irq);
}
...
static irqreturn_t fts_ts_interrupt(int irq, void *dev_id)
{
	...
	if((fts_ts->touch_enable == 1) && (touch_proximity_at_phone == 0))
	{
               if(queue_work(fts_ts->ts_workqueue, &fts_ts->touch_event_work))
	...
	if (on) {
		if (gpio_is_valid(data->pdata->irq_gpio)) {
			err = gpio_request(data->pdata->irq_gpio,
						data->fts_NameList_curr->IrqGpio);
			if (err) {
				dev_err(&data->client->dev,
		...
		if (gpio_is_valid(data->pdata->reset_gpio)) {
			err = gpio_request(data->pdata->reset_gpio,
						data->fts_NameList_curr->ResetGpio);
			if (err) {
				dev_err(&data->client->dev,
...
static int fts_ts_pinctrl_init(struct fts_ts_data *fts_data)
{
	...
	fts_data->pinctrl_state_active
		= pinctrl_lookup_state(fts_data->ts_pinctrl,
				fts_data->fts_NameList_curr->Pinctrl_Active);
...
int fts_ts_suspend(struct device *dev)
{
	...
	//return 0;
#if FTS_GESTRUE_EN
         fts_write_reg(data->client, 0xd0, 0x01);
	  if (data->fts_updateinfo_curr.CHIP_ID==0x54 || data->fts_updateinfo_curr.CHIP_ID==0x58 || data->fts_updateinfo_curr.CHIP_ID==0x86)
	  {
	  	fts_write_reg(data->client, 0xd1, 0xff);
		fts_write_reg(data->client, 0xd2, 0xff);
		fts_write_reg(data->client, 0xd5, 0xff);
		fts_write_reg(data->client, 0xd6, 0xff);
		fts_write_reg(data->client, 0xd7, 0xff);
		fts_write_reg(data->client, 0xd8, 0xff);
	  }
...
static void fts_suspend_work(struct work_struct *work)
{
	struct fts_ts_data *fts_data =
			...
			suspend_work);
	//dev_info(&fts_data->client->dev,  "[FTS] Virtual key suspend start!\n");	
	fts_ts_suspend(&fts_data->client->dev);
	dev_info(&fts_data->client->dev,  "[FTS] Virtual key suspend over!\n");
}
...
static void fts_resume_work(struct work_struct *work)
{
	struct fts_ts_data *fts_data =
			container_of(work, struct fts_ts_data,
			resume_work);
	dev_info(&fts_data->client->dev,  "[FTS] Virtual key resume start!\n");		
	fts_ts_resume(&fts_data->client->dev);
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct fts_NameList *fts_NameList_curr=NULL;
	unsigned int IC_ID = ID_UNKONE;
	struct dentry *temp;
...
//<ASUS-BSP Robert_He 20170313> add capsensor test node ++++++
	...
	if (client->dev.of_node) {
		...
		if(pdata->name)
		{
			if(!strncmp(pdata->name,TOUCH_IC_NAME,TOUCH_IC_NAME_LEN))
			{
				if(synaptics_tp_done)
				{
					dev_err(&client->dev, "synaptics tp have probe!\n");
					return -EINVAL;
				}
				fts_NameList_curr = &fts_ts_NameList;
				IC_ID = ID_FT3517;
				//dev_info(&client->dev,  "This device is Touch IC and Name is %s\n",TOUCH_IC_NAME);
			}
			else if(!strncmp(pdata->name,VIRTUAL_IC_NAME,VIRTUAL_IC_NAME_LEN))
			{
				fts_NameList_curr = &fts_cap_NameList;
				IC_ID = ID_FT3267;
				//dev_info(&client->dev,  "This device is VirtualKey IC and Name is %s\n",VIRTUAL_IC_NAME);
			}
			else
			{
				dev_err(&client->dev, "Can't find device NameList\n");
				return -EINVAL;
			}
		}
		else
		{
			dev_err(&client->dev, "pdata->name is NULL\n");
			return -EINVAL;
		}
...
//<ASUS-BSP Robert_He 20170313> add capsensor global data ++++++
	...
	data->fts_NameList_curr = fts_NameList_curr;
	data->IC_ID = IC_ID;
	...
	data->touch_enable = 1;
	data->glovemode = 0;
	input_dev->name = fts_NameList_curr->InputName;
	...
	data->ts_workqueue = create_workqueue(fts_NameList_curr->IrqWorkQueueName);
...
//<ASUS-BSP Robert_He 20170405> add capsensor vkled function ------
	...
	data->dir = debugfs_create_dir(fts_NameList_curr->DebugDirName, NULL);
	...
	#ifdef FTS_AUTO_UPGRADE
//add by Holt 20160825+
	data->update_fw_wq=create_singlethread_workqueue(fts_NameList_curr->fwWorkQueueName);
	if(!data->update_fw_wq)
	{
		dev_err(&client->dev, "%s:create update firmware woqkqueue failed\n",__func__);
		goto free_debug_dir;
	}
	else
	{
...
//add by Holt 20160825-
	...
	data->suspend_resume_wq = create_singlethread_workqueue(fts_NameList_curr->pmWorkQueueName/*"focal_resume_wq"*/);
	if (!data->suspend_resume_wq) {
		dev_err(&client->dev, "[Focal][TOUCH_ERR] %s: create resume workqueue failed\n", __func__);
		goto free_debug_dir;
	}
...
{
	int ret = 0;
	//if(asus_lcd_id != 3)  return 0;
	ret = i2c_add_driver(&fts_ts_driver);
	return ret;
}
//EXPORT_SYMBOL(fts_ts_init);
/*******************************************************************************
*  Name: fts_ts_exit
*  Brief:
*  Input:
