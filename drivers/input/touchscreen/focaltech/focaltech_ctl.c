...
{
	struct fts_rw_i2c_dev* fts_rw_i2c_dev_tt = container_of(inode->i_cdev,struct fts_rw_i2c_dev,cdev);
...
static long fts_rw_iic_drv_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	mutex_lock(&ftsdev->fts_rw_i2c_mutex);
	switch (cmd)
	{
		case FTS_I2C_RW:
	...
	mutex_unlock(&ftsdev->fts_rw_i2c_mutex);
	return ret;	
}
...
* Return: no
***********************************************************************/
static void fts_rw_iic_drv_setup_cdev(struct fts_rw_i2c_dev *dev, int index, dev_t devno)
{
...
*/
		...
		fts_data->fts_rw_iic_drv_major = MAJOR(devno);
//	}
	if (err < 0) 
	{
	...
	fts_rw_iic_drv_setup_cdev(fts_rw_i2c_dev_tt, 0, devno); 
	fts_data->fts_rw_i2c_dev_tt = fts_rw_i2c_dev_tt;

	fts_data->fts_class = class_create(THIS_MODULE, fts_data->fts_NameList_curr->ClassName);
	if (IS_ERR(fts_data->fts_class)) 
	{
	...
	/*create device node*/
	device_create(fts_data->fts_class, NULL, MKDEV(fts_data->fts_rw_iic_drv_major, 0), 
			NULL, fts_data->fts_NameList_curr->DevName);
...
* Return: no
***********************************************************************/
void  fts_rw_iic_drv_exit(struct i2c_client *client)
{
	struct fts_ts_data *fts_data = i2c_get_clientdata(client);
	
	device_destroy(fts_data->fts_class, MKDEV(fts_data->fts_rw_iic_drv_major, 0)); 
	/*delete class created by us*/
	class_destroy(fts_data->fts_class); 
	/*delet the cdev*/
	cdev_del(&fts_data->fts_rw_i2c_dev_tt->cdev);
	kfree(fts_data->fts_rw_i2c_dev_tt);
	unregister_chrdev_region(MKDEV(fts_data->fts_rw_iic_drv_major, 0), 1); 
}
