...
static int fts_ctpm_get_app_i_file_ver(void)
{
    ...
    if (ui_sz > 2) {	
		//<ASUS-BSP Robert_He 20170323> chip id is 582c which version is 0x10a not ui_se-2 +++++
...
static int fts_ctpm_get_vendor_id(struct i2c_client *client)
{
    ...
    if (fts_updatefun_curr.app_valid == false) {
        ...
        if (i >= FTS_UPGRADE_LOOP) {
            FTS_ERROR
                ("[UPGRADE]: read vendor id from flash more than 30 times!!");
            return -EIO;
        }
		//<ASUS-BSP Robert_He 20170328> add support for vendor id change to 0x01 +++++
        if ((vendorid[1] != FTS_VENDOR_1_ID)
            && (vendorid[1] != FTS_VENDOR_2_ID)
            && (vendorid[1] != FTS_VENDOR_3_ID)) {
            fts_updatefun_curr.need_upgrade = false;
            FTS_ERROR("[UPGRADE]: vendor id error : 0x%x, can not upgrade!!",
                      vendorid[1]);
            return -EIO;
        }
		//<ASUS-BSP Robert_He 20170328> add support for vendor id change to 0x01 -----
...
static int fts_ctpm_get_project_id(struct i2c_client *client)
{
    ...
    if (fts_updatefun_curr.get_projectid) {
        ...
        if (i >= FTS_UPGRADE_LOOP) {
            FTS_ERROR
                ("[UPGRADE]: read project id from flash more than 30 times!!");
            return -EIO;
        }
        ...
        FTS_DEBUG("[UPGRADE]: project id : %s!!",
                  fts_updatefun_curr.project_id);
    }
    return 0;
}
...
//<ASUS-BSP Robert_He 20170323> chip id is 582c which is different from driver 5446 +++++
        else {
            FTS_ERROR
...
//<ASUS-BSP Robert_He 20170328> remove vendor_id and ecc check for app.bin upgrade +++++
