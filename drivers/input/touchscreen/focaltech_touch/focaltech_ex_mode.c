...
//<ASUS-BSP Robert_He 20170328> add usb detect for touch and capsensor charger mode ++++++ 
extern void fts_ts_cap_charger_mode(bool enable);
void touch_usb_detection(bool plugin)
{
	int ret;
	u8 value = 0;
	if (fts_i2c_client == NULL)
	{
		FTS_ERROR("fts_i2c_client is NULL,can not enter charger mode");
		return;
	}
	...
	if (plugin)
	{
		  if (!g_fts_mode_flag.fts_charger_mode_flag) 
		  {
            FTS_INFO("[Mode]enter charger mode for charger detection");
            g_fts_mode_flag.fts_charger_mode_flag = true;
            ret = fts_enter_charger_mode(fts_i2c_client, true);
			if (ret < 0)
			{
			    FTS_ERROR("enter changer mode fail");
			}
		  }
		  fts_ts_cap_charger_mode(1);
	}
	else
	{
		if (g_fts_mode_flag.fts_charger_mode_flag) 
		{
            FTS_INFO("[Mode]exit charger mode for charger detection");
            g_fts_mode_flag.fts_charger_mode_flag = false;
            ret = fts_enter_charger_mode(fts_i2c_client, false);
			if (ret < 0)
			{
		...
		fts_ts_cap_charger_mode(0);
	}
	fts_i2c_read_reg(fts_i2c_client,FTS_REG_CHARGER_MODE_EN,&value);
	FTS_INFO("[robert]this time 0x8B status is %x",value);
}
//<ASUS-BSP Robert_He 20170328> add usb detect for touch and capsensor charger mode  ------
