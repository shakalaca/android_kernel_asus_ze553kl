...
int synaptics_rmi4_link_input2dev(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = sysfs_create_link(&i2c->dev.kobj, &rmi4_data->input_dev->dev.kobj, "devattr");
	if(retval < 0)
	{
		dev_info(&i2c->dev, "[synaptics] Link to absolute path failed\n");
	}
	return retval;
}
...
{
	struct i2c_client *i2c = to_i2c_client(rmi4_data->pdev->dev.parent);
	sysfs_remove_link(&i2c->dev.kobj, "devattr");
}
//<ASUS_link_to_absolute_path->
