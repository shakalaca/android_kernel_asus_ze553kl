...
#define LS_POLLING_DELAY 50//600 asus alex wang ALS polling delay 50ms
...
#define CM36656_VDD_TYP_UV	3300000
...
//asus alex_wang rgb porting-----
#define ASUS_LIGHTSENSOR_DISABLED_LUX 100000 //200lux golden adc=285,1000lux golden adc=1430,maximum lux=65535*200/285=45989lux
...
struct cm36656_info {
  ...
  int als_polling_delay;
	...
	int ps_autok_max;
	#endif
	uint8_t status_calling;//asus 20170424 status calling
	//asus alex_wang psensor autok---
	//uint16_t ls_cmd;
...
//asus alex_wang rgb porting ++++
static struct mutex rgb_enable_mutex,rgb_disable_mutex, rgb_get_adc_mutex;
...
static void lightsensor_value_work_routine(struct work_struct *work);
static DECLARE_DELAYED_WORK(lightsensor_value_work, lightsensor_value_work_routine);
//asus alex_wang ALS porting+++++
...
#endif
//asus alex_wang psensor autok----
//asus alex_wang psensor disable touch++++
...
int LSensor_CALIDATA =0; //input calibration data . Format : "600 lux -->lux value "
//asus alex wang ALS Calibration------
//asus alex wang PS Calibration+++++
bool enPSensorConfig_flag=0;  //the flag of psensor calibration enable
int PSensor_CALIDATA[3] = {0}; //input calibration data . Format : "near 3cm:-->value;far 5cm:-->value;crosstalk:-->value" 
...
static int I2C_RxData(uint16_t slaveAddr, uint8_t cmd, uint8_t *rxData, int length)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		if (loop_i == 0 || loop_i == I2C_RETRY_COUNT -1)
			D("[PS][CM36656 error] %s, i2c err, slaveAddr 0x%x ISR gpio %d  = %d \n",
...
static int I2C_TxData(uint16_t slaveAddr, uint8_t *txData, int length)
{
	...
	for (loop_i = 0; loop_i < I2C_RETRY_COUNT; loop_i++) {
		...
		/*check intr GPIO when i2c error*/
		if (loop_i == 0 || loop_i == I2C_RETRY_COUNT -1)
			D("[PS][CM36656 error] %s, i2c err, slaveAddr 0x%x, value 0x%x, ISR gpio%d  = %d \n",
...
static int _cm36656_I2C_Write_Word(uint16_t SlaveAddress, uint8_t cmd, uint16_t data)
{
	...
	/* Debug use */
	printk(KERN_DEBUG
	"[CM36656] %s: _cm36656_I2C_Write_Word[0x%x, 0x%x, 0x%x]\n",
...
static int get_ps_adc_value(uint16_t *data)
{
	...
	if (ret < 0) {
		pr_err(
			"[PS][CM36656 error]%s: _cm36656_I2C_Read_Word fail\n",
			__func__);
		return -EIO;
	} else {
		pr_err(
			"[PS][CM36656 OK]%s: _cm36656_I2C_Read_Word OK 0x%04x\n",
			__func__, *data);
	}
...
static void read_cs_adc_value(void)
{
	...
	mutex_unlock(&rgb_get_adc_mutex);	
}
...
int calibration_max_lux(int64_t x_big, int64_t x_small)
	{
		static int report_lux = 0xffffffff;
...
static void report_ls_input_event(int report_lux,int report_golden)
{
	...
	if(report_lux != ASUS_LIGHTSENSOR_DISABLED_LUX){
	   cancel_delayed_work(&cct_value_work);
	   queue_delayed_work(lpi->lp_als_wq, &cct_value_work, msecs_to_jiffies(500));
	}
}

//asus alex wang workqueue porting++++

//light sensor workqueue
static void lightsensor_value_work_routine(struct work_struct *work){
	uint16_t adc_value=0;
	int report_lux=0,report_golden = 0;
        int dark_adc=0,bright_adc=0;
	mutex_lock(&CM36656_control_mutex);
	pr_err("[LS][CM36656][lightsensor_wq] start lightsensor wq\n");
        read_ls_adc_value();
	...
	report_golden=read_ls_calibration_value(adc_value,285,1430);//godlen
...
//cct work queue
static int cct_debug_log_count;
static void cct_value_work_routine(struct work_struct *work){
	uint16_t adc_value=0;
	int report_lux=0,report_golden = 0;
        int dark_adc=0,bright_adc=0;
	...
	report_lux = read_ls_calibration_value(adc_value,dark_adc,bright_adc);
	report_golden=read_ls_calibration_value(adc_value,285,1430);//godlen
	if((g_ls_adc_debug==1)||(cct_debug_log_count>100)) {
			 pr_err("[LS][CM36656] %s report lux = %x report_golden = %x R=%x G=%x B=%x IR=%x\n", __func__, report_lux,report_golden,cm36656_adc_red, cm36656_adc_green, cm36656_adc_blue, cm36656_adc_ir);
...
bool proximity_check_status(void){
   ...
   ret = _cm36656_I2C_Read_Word(lpi->slave_addr, PS_CONF1, &ps_conf1_val);
   if(ret<0)
	  goto check_status_error;

   if(ps_conf1_val&CM36656_PS_SD){
   ...
   if(!ret){
   	   if(ps_adc_value > lpi->ps_close_thd_set){
	   	LOGE("[PS][CM36656]%s,status = 1\n",__func__);
	   	  D("[PS][CM36656] proximity initial NEAR\n");
				status = 1; 
	   }else{
	        LOGE("[PS][CM36656]%s,status = 0\n",__func__);
	         D("[PS][CM36656] proximity initial FAR\n");  
				status = 0; }
   }else{
      goto check_status_error; 
   }
	...
	case ASUS_PSENSOR_SETCALI_DATA:
	  ...
	  //asus alex wang PS Calibration++++
	    ...
	    if (copy_from_user(PSensor_CALIDATA, argp, sizeof(PSensor_CALIDATA))){
           ...
           pr_err("[PS][CM36656]%s:ASUS_PSENSOR SETCALI_DATA : PSensor_CALIDATA[0] : %d, PSensor_CALIDATA[1]: %d, PSensor_CALIDATA[2]: %d\n", 
			__func__, PSensor_CALIDATA[0], PSensor_CALIDATA[1], PSensor_CALIDATA[2]); //<asus-wx20160804>
           if(PSensor_CALIDATA[0] <= 0||PSensor_CALIDATA[1] <= 0  ||PSensor_CALIDATA[0] <= PSensor_CALIDATA[1] )
			rc =  -EINVAL;
...
static int lightsensor_disable(struct cm36656_info *lpi)
{
	...
	if ( lpi->als_enable == 0 ) {
		...
		ret = 0;
			 ...
			 cancel_delayed_work(&cct_value_work);
			 cancel_delayed_work(&lightsensor_value_work);
		}
...
static int rgbsensor_disable(struct cm36656_info *lpi)
{
	...
	mutex_unlock(&rgb_disable_mutex);
	//asus rgb timer++++
	#ifndef ASUS_FACTORY_BUILD 
			...
			struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf, "DEC ADC[%d], ENABLE = %d, intr_pin = %d,table_adc=%d\n", value, lpi->ps_enable, intr_val,ps_data);
...
//asus alex wang 20170424 status calling++++
static ssize_t ps_calling_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct cm36656_info *lpi = lp_info;
	return sprintf(buf,"calling status is %d\n", lpi->status_calling);
}
				...
				const char *buf, size_t count)
{
	...
	D("[PS][CM36656]%s: calling status is %d\n", __func__, lpi->status_calling);
	return count;
}
//asus alex wang 20170424 status calling++++
...
*/
static ssize_t ls_pers_show(struct device *dev,
						  struct device_attribute *attr, char *buf)
{
                        struct cm36656_info *lpi = lp_info;
			uint16_t value;
...
static ssize_t ls_pers_store(struct device *dev,
						struct device_attribute *attr,
						const char *buf, size_t count)
{
			int value = 0;
...
static int lightsensor_setup(struct cm36656_info *lpi)
{
	...
	if (!lpi->ls_input_dev) {
		pr_err(
			"[LS][CM36656 error]%s: could not allocate ls input device\n",
			__func__);
		return -ENOMEM;
	}
...
//asus alex_wang rgb porting----
	...
	if (!pdata) {
     ...
     lpi->status_calling = 0;//asus alexwang 20170424 calling status
   //  lpi->ls_cmd  = pdata->ls_cmd;
	...
	lpi->ps_conf1_val = CM36656_PS_IT_2T  | CM36656_PS_PERS_2 | CM36656_PS_START;
	lpi->ps_conf3_val = CM36656_LED_I_110;
       //asus alex_wang ALS sensor porting++++
...
//asus alex_wang rgb porting++++
lpi->rgb_cmd = CM36656_CS_START;
lpi->rgb_cmd &= CM36656_CS_IT_50MS;//setting default RGB IT 50MS
#ifndef ASUS_FACTORY_BUILD
...
//asus alex_wang rgb porting++++
	mutex_init(&rgb_enable_mutex);
	mutex_init(&rgb_disable_mutex);
...
//light
	...
	if (ret)
		goto err_create_ls_device_file;
//psensor
       ret = device_create_file(lpi->ps_dev, &dev_attr_ps_calling);//asus alexwang 20170424 calling status
	if (ret)
...
//asus alex_wang rgb porting++++
err_rgbsensor_setup:
        mutex_destroy(&rgb_enable_mutex);
	mutex_destroy(&rgb_disable_mutex);
...
static int control_and_report( struct cm36656_info *lpi, uint8_t mode, uint16_t param ) {
	...
	if( mode == CONTROL_ALS ){	
		...
		if(param){
			...
			lpi->rgb_cmd |= CM36656_CS_START; 
			lpi->rgb_cmd |= CM36656_ALS_PERS_4;
...
als_pass://asus alex wang ALS sensor porting	
   ...
   if(lpi->ps_enable){
    ...
    if (ps_status!=0){
      switch(ps_status){
        ...
        case PS_CLOSE:
          val = 0;
		  D("[PS][CM36656] proximity detected object close\n");
          break;
        };
	  ...
	  if(proximity_state == 0){
		  if (2 == get_audiomode()) {
			...
			LOGE("[PS][CM36656] proximity -- close  in call \n");
		}else{
			asus_psensor_disable_touch(false);
			LOGE("[PS][CM36656] proximity --away in call \n");
		}	
	}else{
		asus_psensor_disable_touch(false);
		LOGE("[PS][CM36656] proximity --far far awayl \n");
	}
    }
  }
...
static int cm36656_suspend(struct device *dev)
{
	...
	pr_err("CM36656:  cm36656_suspend\n");
	lpi->status_calling = status_calling;
	/*
