...
#define sensitivity_25  25  //zf2 als_it =160ms / 0.04 lux /setp
#define sensitivity_125  125 //zf3 als_it =80ms  /0.08 lux/setp
...
int PSensor_CALIDATA[3] = {0}; //input calibration data . Format : "near 3cm:-->value;far 5cm:-->value;crosstalk:-->value" //<asus-wx20160804>
...
int calibration_max_lux(int64_t x_big, int64_t x_small)
{
	...
	if (report_lux == 0xffffffff) { // only calculate it the first time
		int64_t y_big = 1000 * sensitivity_x / 10; //lux ->adc
		int64_t y_small = 200 * sensitivity_x / 10;
		...
		if (65535 * (y_big - y_small) < constant_k){
			...
			report_lux = (((65535 * (y_big - y_small) - constant_k) * 100 + (x_big - x_small) * sensitivity_x * 5) / 
					((x_big - x_small) * sensitivity_x * 10));			
		}
	}
...
int64_t static calibration_light(int64_t x_big, int64_t x_small, int64_t report_lux){        
	...
	if (report_lux * (y_big - y_small) < constant_k){
		...
		return (((report_lux * (y_big - y_small) - constant_k) * 100 + (x_big - x_small) * sensitivity_x * 5) / 
			((x_big - x_small) * sensitivity_x * 10));		
	}
}

void cal_ls_thd_sel(uint16_t adc_value, uint16_t *ls_low_thd, uint16_t *ls_high_thd, uint16_t low_bound, uint16_t high_bound)
{
	uint32_t temp_adc;
	int current_lux, temp_lux;

	if (enLSensorConfig_flag == 1) {
		current_lux = calibration_light(LSensor_CALIDATA[1], LSensor_CALIDATA[0], (int)adc_value);
	}
	...
	if (enLSensorConfig_flag == 1) {
		...
		do {
			temp_lux = calibration_light(LSensor_CALIDATA[1], LSensor_CALIDATA[0], (int)temp_adc);
...
void cal_ls_thd(uint16_t adc_value, uint16_t *ls_low_thd, uint16_t *ls_high_thd)
{
	if (adc_value == 0) {
		*ls_low_thd = 0;
		*ls_high_thd = 0;
	}
	else if (adc_value <= LSensor_CALIDATA[0]) { //200lux -> adc
		cal_ls_thd_sel(adc_value, ls_low_thd, ls_high_thd, 98, 102);
	}
	else if (adc_value <= LSensor_CALIDATA[1]) { //1000lux -> adc
		cal_ls_thd_sel(adc_value, ls_low_thd, ls_high_thd, 95, 105);
	}
	else {
		cal_ls_thd_sel(adc_value, ls_low_thd, ls_high_thd, 90, 110);
	}
}
//wxtest
...
void psensor_set_threshold(void);
...
struct CM36283_info {
	...
	int last_report_lux; //<asus-wx20160831>
	struct regulator *vdd;
	struct regulator *vio;
//wxtest
	bool als_first_report;
//wxtest
//<asus-wx20160804>+>>
...
//<asus-wx20150805>+<<
	...
	msleep(PROXIMITY_AUTOK_DELAY); //need some delay
...
//wxtest
	cal_ls_thd(adc_value, &ls_low_thd, &ls_high_thd);
//wxtest
...
//wxtest
			        D("CM36283--initial- report_lux is %d\n", report_lux);
					}else{
...
//wxtest
			...
			lpi->last_report_lux = report_lux; //<asus-wx20160831>
}
...
static int psensor_check_minCT(void)
{
	...
	if ((crosstalk_diff > lpi->ps_autok_min) && (crosstalk_diff < lpi->ps_autok_max)) {
		...
		if (crosstalk_diff <= lpi->ps_autok_min) {
			pr_err("[PS][CM36283]psensor_check_minCT : crosstalk_diff = %d, too small\n", crosstalk_diff);
			return -1;
		}
		else {
			lpi->ps_crosstalk_diff = crosstalk_diff;
...
static void proximity_autok(struct work_struct *work)
{
	...
	ret = get_ps_adc_value(&adc_value);
	if (ret < 0) {
		pr_err("[PS][CM36283]proximity_autok : get_ps_adc_value FAIL\n");
		lpi->ps_crosstalk_diff = 0;
		return;
	}
	D("[PS][CM36283]proximity_autok : adc_value = %d\n", adc_value);
	...
	if ((crosstalk_diff < lpi->ps_crosstalk_diff) && (lpi->ps_crosstalk_diff != 0)) {
		...
		if (crosstalk_diff <= lpi->ps_autok_min) {
			...
			lpi->ps_crosstalk_diff = 0;
			pr_err("[PS][CM36283]proximity_autok : crosstalk is normal, stop poll psensor autok\n");
		}
		else if ((crosstalk_diff > lpi->ps_autok_min) && (crosstalk_diff < lpi->ps_autok_max)) {
			lpi->ps_crosstalk_diff = crosstalk_diff;
			pr_err("[PS][CM36283]proximity_autok : update ps_crosstalk_diff = %d\n", lpi->ps_crosstalk_diff);
			...
			lpi->ps_crosstalk_diff = crosstalk_diff;
			pr_err("[PS][CM36283]proximity_autok : crosstalk_diff = %d, too big\n", crosstalk_diff);
		}
	}
}
//<asus-wx20160804>+<<
...
//<asus-wx20160831>+>>
	input_report_abs(lpi->ls_input_dev, ABS_MISC, ASUS_LIGHTSENSOR_DISABLED_LUX);
	input_sync(lpi->ls_input_dev);
//<asus-wx20160831>+<<
	return ret;
}
...
#endif // end of CONFIG_ASUS_FACTORY_SENSOR_MODE				
					     //report_lux = report_lux*10/sensitivity_x;
	                     D("CM36283---, ASUS_LIGHTSENSOR_IOCTL_GETDATA, adc value is %d\n", report_lux);	
	              }
                               ...
                               const char *buf, size_t count)
{
	...
	pr_err("[LS][CM36283] set g_ls_adc_debug = %d\n", g_ls_adc_debug);

	return count;
}

static DEVICE_ATTR(ls_adc_debug, 0664, ls_adc_debug_show, ls_adc_debug_store);
//<asus-wx20160831>+<<
...
//<asus-wx20160831>+>>
	ret = device_create_file(lpi->ls_dev, &dev_attr_ls_adc_debug);
...
//<asus-wx20160831>+>>
		if (!lpi->als_enable) {
			pr_err("[LS][CM36283]light sensor last report lux = %d\n", lpi->last_report_lux);
		}
//<asus-wx20160831>+<<
	        }
  ...
  if(mode == CONTROL_PS){  
        if( param==1 ){
			//<ASUS-Lotta_Lu-20150603-Add for reduce enable Psensor Time>
		     msleep(PROXIMITY_AUTOK_DELAY);  //<asus-wx20160804>
//<asus-wx20160804>+>>
		...
		if (lpi->ps_autok_module_enable) {
			...
			if (ret < 0) {
				pr_err("[PS][CM36283]proximity_check_minCT ERROR\n");
...
//wxtest
if (report_lux > 0) {
                          //D("CM36283---adc is %d, low_thd is %d, high_thd is %d\n",report_lux,ls_low_thd,ls_high_thd);
                          report_lux = calibration_light(LSensor_CALIDATA[1], LSensor_CALIDATA[0], report_lux);
                          //D("CM36283---After calibration, report_lux is %d\n", report_lux);
                          //report_lux = (report_lux*100 + 625)/(sensitivity_x*10); //wxtest
			  //D("CM36283---report_lux is %d\n", report_lux);
			if (report_lux == 0) report_lux = 1;
}
else {
			report_lux = 0;
...
//wxtest
                  ...
                  D("CM36283--- NO calibration data, Factory branch , NO default config\n"); 				  
#endif // end of CONFIG_ASUS_FACTORY_SENSOR_MODE
                  report_lux = report_lux*10/sensitivity_x*(200/15);
  		  //        D("CM36283---After devided by 20, report_lux is %d\n", report_lux);	
		//<-- ASUS-Bevis_Chen - -->
             }			

	 if ((g_ls_adc_debug) || (debug_log_count > 10)) { //<asus-wx20160831>
		debug_log_count = 0;

    		D("[LS][CM36283] adc=%d, l_thd=%d, h_thd=%d, report_lux=%d\n",adc_value,ls_low_thd,ls_high_thd,report_lux);
	  }else{
