	...
	enum msm_vfe_input_src frame_src, struct msm_isp_timestamp *ts)
{
	...
	//asus bsp ralf: for workaround AEC not working after camera switch >>
	if(vfe_dev->axi_data.src_info[frame_src].frame_id>vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id){
		dev_info(&vfe_dev->pdev->dev,"%s [src=%d,fid=%d]>[src=%d,fid=%d]",__func__
			,frame_src,vfe_dev->axi_data.src_info[frame_src].frame_id
			,VFE_PIX_0,vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id	);
		vfe_dev->axi_data.src_info[frame_src].frame_id=vfe_dev->axi_data.src_info[VFE_PIX_0].frame_id;
	}
	//asus bsp ralf: for workaround AEC not working after camera switch <<
	...
	enum msm_vfe_hw_state hw_state)
{
	...
	total_bandwidth = total_pix_bandwidth + total_rdi_bandwidth +
			total_fe_bandwidth;
	//asus bsp ralf:add for reduce isp overflow,recommanded by qcom gary>>
	total_bandwidth*=3;
	//asus bsp ralf:add for reduce isp overflow,recommanded by qcom gary<<
