...
int Verify_ID_is_Correct(struct msm_laser_focus_ctrl_t *dev_t, int chip_id_size){
	...
	if (chip_id != slave_info->sensor_id) {
		if(chip_id == 0x1AD || chip_id == 0x2AD)
		{
			LOG_Handler(LOG_ERR, "%s: For Olivia 0.0 and 1.0 FW IC, pass check!\n", __func__);
			return rc;
		}
...
void Product_Family(struct msm_laser_focus_ctrl_t *dev_t){
	...
	if(i == CPE_LEN){
		LOG_Handler(LOG_DBG,"Laser Product is OLIVIA\n");
		Laser_Product = PRODUCT_OLIVIA;
		if(PV == '0')
		{
			FirmWare = Laser_Product;
			LOG_Handler(LOG_DBG, "%s: Laser FW is OLIVIA\n", __func__);
		}
		else
			LOG_Handler(LOG_DBG, "%s: unknown FW version: %c(%d), should be 0(%d)\n", __func__, PV,PV,'0');
	}
...
extern void Olivia_read_FW_ChipID(struct msm_laser_focus_ctrl_t *dev_t);
...
int dev_I2C_status_check(struct msm_laser_focus_ctrl_t *dev_t, int chip_id_size){
	...
	if(dev_t->device_state == MSM_LASER_FOCUS_DEVICE_OFF ||dev_t->device_state == MSM_LASER_FOCUS_DEVICE_DEINIT_CCI )
	{
		LOG_Handler(LOG_DBG, "%s: full test\n", __func__);
		rc =power_up(dev_t);
				if (rc < 0) {
	...
	else if(dev_t->device_state == MSM_LASER_FOCUS_DEVICE_NO_APPLY_CALIBRATION ||
			dev_t->device_state == MSM_LASER_FOCUS_DEVICE_APPLY_CALIBRATION ||
			dev_t->device_state == MSM_LASER_FOCUS_DEVICE_INIT_CCI
		  )
	{
		LOG_Handler(LOG_DBG, "%s: just read test\n", __func__);
		Laser_Match_Module(dev_t);
	}
