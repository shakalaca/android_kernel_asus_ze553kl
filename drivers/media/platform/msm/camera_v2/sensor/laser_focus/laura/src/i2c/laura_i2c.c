...
int Laura_device_indirect_addr_write(struct msm_laser_focus_ctrl_t *dev_t,
	...
	Laura_write_seq(sensor_i2c_client, i_reg_addr_lsb, i_reg_addr_msb, indirect_addr, register_addr, i2c_write_data, num_word);
	if (status < 0) {
...
*
*/
int32_t Laura_write_seq(struct msm_camera_i2c_client *client, 
							uint16_t i_reg_addr_lsb, uint16_t i_reg_addr_msb, uint16_t indirect_addr, 
							uint32_t reg_addr, uint16_t *data, uint32_t num_word)
{
	int32_t rc = -EFAULT;
	uint8_t i = 0, j = 0;
	int reg_conf_tbl_size = (num_word*2)+2;//+2 for address info
