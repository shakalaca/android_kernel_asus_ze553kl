...
uint16_t Laura_device_read_range2(struct msm_laser_focus_ctrl_t *dev_t)
{
	...
	while(1){
		...
		LOG_Handler(LOG_CDBG, "%s: Read raw range:%d\n", __func__,(RawRange&DISTANCE_MASK)>>2);
		//bit 14:13
...
*
*/
int Laura_device_calibration(struct msm_laser_focus_ctrl_t *dev_t, uint16_t *cal_input_data)
{
	int status = 0, pass_count = 0, fail_count = 0, i = 0, j = 0;
	uint16_t distance = 0;
	uint16_t cal_data[CAL_MSG_LEN];
...
*
*/
int Laura_Create_Calibration_Data(int16_t *cal_10, int16_t *cal_40, int16_t *cal_inf){
	int status = 0;
	int16_t *cal_data;
	...
	Laura_Write_Calibration_Data_Into_File(cal_data, SIZE_OF_LAURA_CALIBRATION_DATA);
...
*
*/
int Laura_Write_Calibration_Data_Into_File(int16_t *cal_data, uint32_t size){
	int status = 0;
...
*
*/
int Laura_Read_Calibration_Data_From_File(uint16_t *cal_data, uint32_t size){
	int status = 0, i = 0;
	int buf[size];

	LOG_Handler(LOG_FUN, "%s: Enter\n", __func__);
#if 0
	if(CSCTryGoldenRunning)
	{
		LOG_Handler(LOG_CDBG, "%s: loading CSC Kdata file %s\n", __func__,LAURA_CALIBRATION_CSC_FILE);
		status = Sysfs_read_word_seq(LAURA_CALIBRATION_CSC_FILE, buf, size);
	}
	else
#endif
...
extern int16_t cal_data_confidence[CAL_CONFIDENCE_LEN];
int Olivia_get_calibration_input(struct seq_file *buf, void *v,
	uint16_t *cal_data_10, uint16_t *cal_data_40,uint16_t *cal_data_f0, uint16_t * ConfPoint)
{
...
//ComputeConfidencePoint is assumed to be in MCPU on
int ComputeConfidencePoint(struct msm_laser_focus_ctrl_t *dev_t)
{
	uint16_t RawRange = 0;
	uint16_t RawConfidence = 0, confidence_level =0;
	int confidenceA=0,confidenceC=0;
	int status;
	...
	if(status==0)
	status = (RawRange&(ERROR_CODE_MASK|VALID_DATA))!=VALID_DATA;
	...
	if(status==0){
		   ...
		   confidenceA = 9*confidence_level/10;
		   confidenceC = confidenceA*6500/(6500-Settings[IT]);
	}

	if(status==0){
		   cal_data_confidence[0] = confidenceA;
		   cal_data_confidence[1] = confidenceC;
	}
	else{
		   cal_data_confidence[0] = Settings[CONFIDENCE10];
		   cal_data_confidence[1] = Settings[CONFIDENCE_THD];
	}
	LOG_Handler(LOG_CDBG, "calibrating Confidence A/C %d/%d\n",confidenceA, confidenceC);
	return status;
}
