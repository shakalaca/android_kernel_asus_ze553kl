...
int Olivia_device_tof_configuration_interface(struct msm_laser_focus_ctrl_t *dev_t, int ctrl){
	...
	switch(ctrl){
		/* These are set TOF configuration */
		case MSM_LASER_FOCUS_APPLY_NORMAL_CALIBRATION://0	LAURA_READ_CONFIG
			status = Laura_device_UpscaleRegInit(dev_t, config_normal);
			break;
		case MSM_LASER_FOCUS_APPLY_OFFSET_CALIBRATION://1   LAURA_CALIBRATION_10_CONFIG
			status = Laura_device_UpscaleRegInit(dev_t, config_K10);
			break;
		case MSM_LASER_FOCUS_APPLY_CROSSTALK_CALIBRATION://2 LAURA_CALIBRATION_40_CONFIG
