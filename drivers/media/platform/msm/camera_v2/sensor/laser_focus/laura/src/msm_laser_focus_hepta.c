...
#if 0
//#ifdef ASUS_FACTORY_BUILD
	seq_printf(buf,"Factory Image, measure mode is fixed to %d, %s\n",
					continuous_measure,
					continuous_measure?"Continuous Measure Mode":"SingleShot Measure Mode"
				);
#else
	mutex_ctrl(laura_t, MUTEX_LOCK);
	...
	if(laura_t->device_state != MSM_LASER_FOCUS_DEVICE_OFF)
	{
		...
		else//wait measure work stop
		{
			...
			LOG_Handler(LOG_ERR,"%s(), measure work stop!\n",__func__);
		}
	}
	seq_printf(buf,"measure mode now is %d, %s\n",
					continuous_measure,
					continuous_measure?"Continuous Measure Mode":"SingleShot Measure Mode"
				);
