...
/*
	...
	if(ret)
		printk("[sean] : error to gpio_request\n");

	ret = gpio_direction_output(46,1);
	if(ret)
		printk("[sean] : error to gpio_direction_output\n");

	gpio_set_value(46, 0);
	//sean_lu -- for gpio46 set output
	
	//sean_lu ++ for gpio47 set output
	ret = gpio_request(47,"GPIO46");
	if(ret)
		printk("[sean] : error to gpio_request\n");
