...
#undef  pr_fmt
#define pr_fmt(fmt) " [sz_cam_ois] I2C %s() " fmt, __func__
...
int rumba_write_dword(struct msm_ois_ctrl_t * ctrl,uint16_t reg_addr, uint32_t reg_data)
{
	...
	//MSB in higher address
	reg_setting->reg_data[3] = (uint8_t)((reg_data & 0xFF000000) >> 24);
	reg_setting->reg_data[2] = (uint8_t)((reg_data & 0x00FF0000) >> 16);
	reg_setting->reg_data[1] = (uint8_t)((reg_data & 0x0000FF00) >> 8);
	reg_setting->reg_data[0] = (uint8_t)(reg_data & 0x000000FF);
