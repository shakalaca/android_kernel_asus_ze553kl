...
static int initialise_recv(struct iris_device *radio)
{
    ...
    //change for fm "small big sound" when switch to speaker in 91.1MHz using record as source  --begin
    //radio->mute_mode.soft_mute = CTRL_ON;
	radio->mute_mode.soft_mute = CTRL_OFF;
	FMDERR("%s, soft mute is disabled!\n", __func__);
	//change for fm "small big sound" when switch to speaker in 91.1MHz using record as source  --end
