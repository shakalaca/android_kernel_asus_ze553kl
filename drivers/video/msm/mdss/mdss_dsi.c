...
//<ASUS-BSP Hank2_Liu 20160420> Add Boe panel iovcc&+/-5v still on ++++++	
	if(((!(gesture_mode&0x40)) && (dclick_mode == 0) && (swipe_mode == 0)) || (ASUS_ZE553KL == asus_project_id)){
...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	ctrl_pdata = container_of(pdata, struct mdss_dsi_ctrl_pdata,
				panel_data);
	if(cabc_first_boot || ((!(gesture_mode&0x40)) && (dclick_mode == 0) && (swipe_mode == 0)) || (ASUS_ZE553KL == asus_project_id)){
		ret = msm_dss_enable_vreg(
			ctrl_pdata->panel_power_data.vreg_config,
