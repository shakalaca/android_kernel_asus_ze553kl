...
disable_vdd:
	...
	if (ret) dev_err(qphy->phy.dev, "Unable to disable vdd:%d\n",ret);

unconfig_vdd:
	ret = qusb_phy_config_vdd(qphy, false);
	if (ret) dev_err(qphy->phy.dev, "Unable unconfig VDD:%d\n",ret);
*/
...
static int qusb_phy_linestate_with_idp_src(struct usb_phy *phy)
{
	...
	writel_relaxed(0x0, qphy->base + QUSB2PHY_PORT_QUICKCHARGE1); //***ADD 
 	writel_relaxed(0x30, qphy->base + QUSB2PHY_PORT_QUICKCHARGE2); //***ADD
...
static int qusb_phy_probe(struct platform_device *pdev)
{
	...
	switch (asus_rf_id)
	{
		...
		case ASUS_ID_IN:
		case ASUS_US_BR:
			dev_dbg(dev, "asus_rf_id is %d , Use NOCA USB init seq \n",asus_rf_id);
			of_get_property(dev->of_node, "qcom,qusb-phy-init-seq-noca-device", &size);
			if (size) {
		...
		case ASUS_TW_JP:
		case ASUS_CN6:
			dev_dbg(dev, "asus_rf_id is %d , Use CA USB init seq \n",asus_rf_id);
			of_get_property(dev->of_node, "qcom,qusb-phy-init-seq-ca-device", &size);
			if (size) {
				qphy->qusb_phy_init_seq = devm_kzalloc(dev,
								size, GFP_KERNEL);
				if (qphy->qusb_phy_init_seq) {
	...
	ret = qusb_phy_config_vdd(qphy, true);
	if (ret) dev_err(qphy->phy.dev, "Unable to config VDD:%d\n", ret);

	ret = regulator_enable(qphy->vdd);
	if (ret) dev_err(qphy->phy.dev, "Unable to enable VDD\n");
